...
var businessRoleEditInit = function () {
...
var submitForm = function (type) {
    ...
    var businessRoleData = $('#editForm').serialize();
    var url = root + "/businessRole/" + type + "BusinessRole.do";
    $.post(url, businessRoleData, function (response) {
        if (response.code == 1) {
            parent.layer.msg('操作成功', {time: 1500, icon: 1});
            parent.businessRoleManage.refreshBusinessRole();
            parent.layer.close(index);
        } else {
            if (response.msg)
        ...
        fields: {
            name: {
                validators: {
                    ...
                    remote: {
                        message: "业务角色名已存在.",
                        url: root + "/businessRole/checkBusinessRoleNameUnique.do",
            ...
            code: {
                validators: {
                    ...
                    remote: {
                        message: "业务角色编码已存在.",
                        url: root + "/businessRole/checkBusinessRoleCodeUnique.do",
                        data: function (validator, $field, value) {
                            return {
                                fid: $('#editForm input[name="fid"]').val(),
