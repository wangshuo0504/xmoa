...
public class ColorLineChart {
	...
	public ChartPanel createChart() {
		// 2：创建Chart[创建不同图形]
		JFreeChart chart = ChartFactory.createLineChart("Monthly Average Rainfall", "", "Rainfall (mm)", createDataset(), PlotOrientation.VERTICAL, true, true, true);
		// 3:设置抗锯齿，防止字体显示不清楚
...
//		Color yanzhong = new Color(255, 1, 1);
		...
		colors.add(new Color(2, 157, 3));
		colors.add(new Color(148, 211, 81));
		colors.add(new Color(255, 255, 4));
		colors.add(new Color(246, 185, 0));
		colors.add(new Color(255, 1, 1));
		ChartUtils.setColorYAixs(chart.getCategoryPlot(),0,500,colors);// Y坐标轴渲染

		// 设置背景图片
//		Image image = null;
//		try {
//			image = ImageIO.read(new File("C:\\Users\\Administrator\\Desktop\\新建文件夹 (2)\\Desert.jpg"));
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
//		chart.setBackgroundImage(image);
		...
		//转换base64编码
		String xml_img = ChartUtils.toBase64(chart, 700, 380);
		System.out.println(xml_img);
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				// 创建图形
				ChartPanel chartPanel = new ColorLineChart().createChart();
